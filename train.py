...
def validation(epoch,best_pred, loss_factor_map, val_loss_factor_map):
    danet.eval()
    evaluator.reset()
    test_loss = 0.0
    ...
    if new_pred > best_pred:
        ...
        best_pred = new_pred
        torch.save(danet,'./danet_100epoch.pth'.format(NUM_loss_map))
...
for epoch in range(args.epochs):
    ...
    #if (epoch+1) % 2 ==0:
    if (epoch+1) % 1 ==0:
        best_pred, v_loss, v_acc, Adjusted_mIoU, Other_mIoU = validation(epoch,best_pred, loss_factor_map, val_loss_factor_map)
...
train_df.to_pickle('./training_data/Top_{}_{}_100epoch.pkl'.format(NUM_loss_map, round(best_pred, 4)))
