...
danet_baseline = get_danet()
danet_baseline = danet_baseline.to(device)

weight = None
criterion = SegmentationLosses(weight=weight, cuda=args.cuda).build_loss(mode='ce')
optimizer = torch.optim.SGD(danet_baseline.parameters(), lr=args.lr, momentum=0, weight_decay=args.weight_decay)
...
def train(epoch, optimizer, train_loader):
    ...
    for iteration, batch in enumerate(train_loader):
        ...
        # grad = 0
        danet_baseline.zero_grad()

        inputs = Variable(inputs)
        labels = Variable(labels)
        #print(input.shape)
        out = danet_baseline(inputs)
        out = out[0]
...
def validation(epoch,best_pred, loss_factor_map):
    danet_baseline.eval()
    evaluator.reset()
    test_loss = 0.0
    ...
    if new_pred > best_pred:
        ...
        best_pred = new_pred
        torch.save(danet_baseline,'./training_data/Model_baseline_100epcoh.pth'.format(NUM_loss_map))
...
for epoch in range(args.epochs):
    ...
    if (epoch+1) % 1 ==0:
    #if (epoch+1) % 2 ==0:
        best_pred, v_loss, v_acc, Adjusted_mIoU, Other_mIoU = validation(epoch,best_pred, val_loss_factor_map)
...
train_df.to_pickle('./training_data/Baseline_{}_{}.pkl'.format(NUM_loss_map, round(best_pred, 4)))
